...
result_df %>% 
  ...
  summarize(n = n(),
            p = n / nrow(result_df))
```
Процент накрытых не согласуется с заявленной вероятностью 95%
...
```{r cauchy_test, message=FALSE}
library(goft)
cauchy_test(result_df$p_dif)
```

Похоже, что это не распределение Коши.
...
result_df %>% 
  ...
  summarize(n = n(),
            p = n / nrow(result_df))
```
При уменьшении разности вероятностей меняется процент накрытых доверительными интервалами.
...
result_df %>% 
  ...
  summarize(n = n(),
            p = n / nrow(result_df))
```
А почти максимальной разницу p_R и p_T, ненакрытие становится равно 100%.
...
result_df %>% 
  ...
  summarize(n = n(),
            p = n / nrow(result_df))
```
При значительной разницу p_R и p_T, вероятность накрытия опять другая.
...
result_df %>% 
  ...
  summarize(n = n(),
            p = n / nrow(result_df))
```
Вероятность накрытия имеет сложную зависимость от соотношения вероятностьей p_R и p_T.
...
Создадим функцию для эксперимента
```{r do_experiment_2}
...
Выполняем эксперимент 1000 раз (p_R = 0.1, p_T = 0.2)
```{r experiment_2}
num_repeats = 1000
result_df <- lapply(1:num_repeats, function(i) do_experiment(p_R = 0.1, p_T = 0.2)) %>% bind_rows()
result_df %>% head()
...
```
Результат согласуется с заявленной вероятностью 95%
...
```
Это не логнормальное распределение (хотя похоже)

Выполняем эксперимент 1000 раз (p_R = 0.1, p_T = 0.15)
```{r res_fisher_3}
num_repeats = 1000
result_df <- lapply(1:num_repeats, function(i) do_experiment(p_R = 0.1, p_T = 0.15)) %>% bind_rows()
result_df %>% 
...
Выполняем эксперимент 1000 раз (p_R = 0.1, p_T = 0.9)
```{r res_fisher_4}
num_repeats = 1000
result_df <- lapply(1:num_repeats, function(i) do_experiment(p_R = 0.1, p_T = 0.15)) %>% bind_rows()
result_df %>% 
...
Выполняем эксперимент 1000 раз (p_R = 0.1, p_T = 0.5)
```{r res_fisher_5}
num_repeats = 1000
...
Выполняем эксперимент 1000 раз (p_R = 0.5, p_T = 0.6)
```{r res_fisher_6}
num_repeats = 1000
...
Видимо, методика построения ДИ, реализованная в fisher.test, при различных значениях p_R и p_T работает не корректно.