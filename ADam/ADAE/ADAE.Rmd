...
```
## Подгружаем библиотеки и читаем файлы
...
## Содаем заготовку ADAE
```{r}
ADAE <- ADSL
ADAE <- left_join(ADAE, AE_ADVERSEEVENTS, 'SUBJID') 
ADAE$STUDYID <- "XXXXX-XX-XX" # костыль
ADAE <- ADAE %>% 
...
## Создадим ряд полесных функций

Функция для AEBODSYS
```{r}
...
## Изменим и добавим столбцы в соответствии со спецификацией
```{r}
ADAE <- ADAE %>% 
...
## Сохраним нужные столбцы в нужном порядке в ADAE.xlsx
```{r}
ADAE %>% 
  select(
